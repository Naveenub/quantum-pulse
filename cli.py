...
def unseal(
    ...
    async def _run():
        ...
        if output:
            raw = bytes(payload["data"]) if isinstance(payload.get("data"), list) else json.dumps(payload).encode()
        ...
        with Progress(
            ...
            async for manifest in scanner.scan():
                ...
                blob, meta = await engine.seal(
                    manifest.model_dump(), pulse_id=pid,
                    parent_id=master_id,
...
def benchmark(
    ...
    async def _run():
        ...
        samples = [
            _json.dumps({"shard": i, "rows": list(range(rows))}).encode()
            for i in range(200)
        ]
        await engine.bootstrap_dict(samples)
